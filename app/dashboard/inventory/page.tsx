...
export default function InventoryPage() {
  ...
  const [newVariant, setNewVariant] = useState({
    productName: "",
    category: DEFAULT_CATEGORIES[0] as ProductCategory,
    image: "",
    barcode: "",
    size: "",
    color: "",
  ...
  const addCustomCategory = (category: string, isEdit: boolean = false) => {
    ...
    // Set the category in the appropriate form
    if (isEdit) {
      setEditForm(prev => ({ ...prev, category: trimmedCategory as ProductCategory }));
      setEditShowCustomCategory(false);
      setEditCustomCategory("");
    } else {
      setNewVariant(prev => ({ ...prev, category: trimmedCategory as ProductCategory }));
  ...
  const handleAddVariant = async () => {
    const finalCategory = newVariant.category;
  ...
  const handleEditVariant = async () => {
    ...
    const finalCategory = editForm.category;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button 
            ...
            disabled={refreshing}
            className="flex items-center gap-2 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div>
                ...
                {showCustomCategory ? (
                  <div className="flex gap-2">
                    ...
                    />
                    <button 
                      onClick={() => addCustomCategory(customCategory)} 
                      className="rounded-md bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-500 flex items-center gap-1"
                      title="Add category"
                    >
                    ...
                    <button 
                      ...
                      className="rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
                      title="Cancel"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  ...
                  <div className="flex gap-2">
                    <select 
                      ...
                      onChange={(e) => setNewVariant({ ...newVariant, category: e.target.value as ProductCategory })} 
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          addCustomCategory(editCustomCategory, true);
                        }
                      }}
                      placeholder="Enter new category" 
                      className={inputClass} 
                    ...
                    />
                    <button 
                      onClick={() => addCustomCategory(editCustomCategory, true)} 
                      className="rounded-md bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-500 flex items-center gap-1"
                      title="Add category"
                    >
                      <Check className="h-4 w-4" />
                    ...
                    <button 
                      onClick={() => { setEditShowCustomCategory(false); setEditCustomCategory(""); }} 
                      className="rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
                      title="Cancel"
                    >
                      <X className="h-4 w-4" />
