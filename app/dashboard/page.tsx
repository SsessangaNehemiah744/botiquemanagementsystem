...
export default function DashboardPage() {
  ...
  const [activePopup, setActivePopup] = useState<"revenue" | "items" | "lowStock" | "customers" | "inventory" | null>(null);
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-md bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 p-4 text-sm text-red-600 dark:text-red-400">
          ...
          <p>{error}</p>
          <button onClick={fetchDashboardData} className="mt-2 underline font-medium">Retry</button>
        </div>
      )}
      ...
      <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
        ...
        {sortedInventory.length === 0 ? (
          <div className="text-center py-8 text-slate-400">
            ...
            <p className="text-xs mt-1">Recently added products will appear here</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800/50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-medium text-slate-600 dark:text-slate-400">Size/Color</th>
                  <th className="px-4 py-2 text-left font-medium text-slate-600 dark:text-slate-400">Design</th>
                  <th className="px-4 py-2 text-right font-medium text-slate-600 dark:text-slate-400">Price</th>
                  <th className="px-4 py-2 text-right font-medium text-slate-600 dark:text-slate-400">Stock</th>
                  <th className="px-4 py-2 text-left font-medium text-slate-600 dark:text-slate-400">Added</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {sortedInventory.slice(0, 5).map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    ...
                    <td className="px-4 py-2 text-xs text-slate-500">{item.design || "—"}</td>
            ...
            {sortedInventory.length > 5 && (
              <p className="text-xs text-center text-slate-400 py-2">
                Showing 5 of {sortedInventory.length} items. Click "View All" for full list.
              </p>
            )}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-4xl rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-lg font-bold">📦 Inventory Added ({sortedInventory.length} items)</h3>
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search inventory by name, category, size, color, or barcode..."
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 p-3 flex justify-between items-center">
              <span className="text-xs text-slate-400">
                {sortedInventory.length} item{sortedInventory.length !== 1 ? "s" : ""}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {sales.length === 0 ? (
                <p className="text-center text-slate-500 py-8">No sales recorded today.</p>
              ) : (
                <div className="space-y-3">
                  ...
                  {sales.map((sale) => (
                    <div key={sale.id} className="rounded-md border border-slate-200 dark:border-slate-700 p-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-sm">Sale #{sale.id.substring(0, 8)}</p>
                          <p className="text-xs text-slate-500">
                            {new Date(sale.created_at).toLocaleTimeString()} · {sale.payment_method}
                          </p>
                        </div>
      ...
      {activePopup === "items" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-lg font-bold">Items Sold Today & Stock Levels</h3>
              <button onClick={closePopup} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-5 w-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              {totalItems === 0 ? (
                <p className="text-center text-slate-500 py-8">No items sold today.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left font-medium">Size/Color</th>
                        ...
                        <tr key={item.id}>
                          ...
                          <td className="px-4 py-2 text-center">
                            <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                              item.stock === 0 ? "bg-red-50 text-red-600 dark:bg-red-500/20 dark:text-red-400" : 
                              item.stock <= item.threshold ? "bg-yellow-50 text-yellow-600 dark:bg-yellow-500/20 dark:text-yellow-400" : 
                              "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400"
                            }`}>
                              {item.stock === 0 ? "Out" : item.stock <= item.threshold ? "Low" : "OK"}
                            </span>
                          </td>
                        </tr>
                      ))}
          ...
          <div className="w-full max-w-lg rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {lowStockItems.length === 0 ? (
                <p className="text-center text-slate-500 py-8">All items are well stocked! 🎉</p>
              ) : (
                <div className="space-y-2">
                  {lowStockItems.map((item: any) => (
                    <div key={item.id} className="flex items-center justify-between rounded-md border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-500/5 p-3">
                      <div>
                        <p className="font-medium text-sm">{item.products?.name || "Unknown"}</p>
                        <p className="text-xs text-slate-500">{item.size} / {item.color}</p>
                      </div>
                      <div className="text-right">
                        <p className={`font-bold text-sm ${item.stock_quantity === 0 ? "text-red-600" : "text-yellow-600"}`}>
                          {item.stock_quantity} left
                        </p>
                        <p className="text-xs text-slate-400">Threshold: {item.low_stock_threshold}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {activePopup === "customers" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-lg font-bold">👥 Customers Served Today</h3>
              <button onClick={closePopup} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-5 w-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              {customersServed.length === 0 ? (
                <p className="text-center text-slate-500 py-8">No customers recorded today.</p>
              ) : (
                <div className="space-y-2">
                  {customersServed.map((customer: any) => (
                    <div key={customer.id} className="flex items-center justify-between rounded-md border border-slate-200 dark:border-slate-700 p-3">
